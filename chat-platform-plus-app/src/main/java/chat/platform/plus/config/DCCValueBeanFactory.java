...
import chat.platform.plus.types.annotations.DCCValue;
...
public class DCCValueBeanFactory implements BeanPostProcessor {
    ...
    private static final String BASE_CONFIG_PATH = "chat_platform_dcc_";
    ...
    public RTopic dccTopicListener(RedissonClient redissonClient) {
        // 创建主题
        RTopic topic = redissonClient.getTopic("chat_platform_dcc");
        // 添加监听器
        topic.addListener(String.class, (charSequence, s) -> {
            // s 的值为 dccTopic.publish(key + "," + value); 中的 key + "," + value
            // 分别获取 key 和 value
    ...
    public Object postProcessAfterInitialization(Object bean, String beanName) throws BeansException {
        ...
        if (AopUtils.isAopProxy(bean)) {
            // 获取动态代理对象的目标类和目标对象
            // 因为动态代理对象只是负责执行委托对象（目标对象）的方法，不包含目标对象的属性字段
